...
\begin{itemize}
	...
	\item \href{https://invensense.tdk.com/wp-content/uploads/2015/02/MPU-6000-Register-Map1.pdf}{TDK MPU6050: конфигурационные регистры}
...
Процессор на плате содержит три контролера I2С, однако воспользоваться возможно только одним, так как выводы двух оставшихся заняты контроллером карты памяти (sdhci), и при их назначении на I2C интерфейс, ОС Linux не сможет загрузится.

\subsection{} Создайте рабочую папку в виртуальной машине для создания наложенного дерева устройств и перейдите в неё 
\begin{lstlisting}[style=bash]
...
\subsection{} Скомпилируйте и отправьте файл на плату
\begin{lstlisting}[style=bash]
...
\subsection{} Исправьте файл barebox.sh
\begin{lstlisting}[style=bash]
...
\subsection{}После перезагрузки выполните команду
\begin{lstlisting}[style=bash]
$ ls /dev/i2c*
\end{lstlisting} 

В выдачи должен присутствовать файл i2c-0. Если его нет, выполните команду dmesg на целевой платформе, и изучите лог загрузки на предмет ошибок инициализации I2C контроллера, при их отсутствии, перезагрузите плату, и изучите лог работы загрузчика, на пример ошибок в наложенном дереве устройств.
...
\subsection{} Воспользуйтесь командой i2cdetect для поиска адресов подключенных  к выбранной I2C шине устройства:
\begin{lstlisting}[style=bash]
# i2cdetect -y 0
\end{lstlisting}
...
\subsection{} Скопируйте пример программы работающей с I2C и откройте её в редакторе vscode:
...
\subsection{} Скомпилируйте приложение
\begin{lstlisting}[style=bash]
# make
\end{lstlisting}

\subsection{} Если компиляция прошла успешно, отправьте полученное приложение на плату:
...
\subsection{} Запустите приложение на плате
\begin{lstlisting}[style=bash]
$ /home/netuser/app
\end{lstlisting}
