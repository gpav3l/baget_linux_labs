...
\subsection{} Для продолжения установки, понадобиться утилита qemu позволяющая эмулировать различные архитектуры. Создайте символьную ссылку на него в директории rootfs (в противном случае нужно было бы удалить этот файл перед переносом на пату):
\begin{lstlisting}[style=bash]
# sudo ln -s /usr/bin/qemu-mips64el-static ./rootfs/usr/bin/
\end{lstlisting}
...
openssh-server resolvconf hwinfo net-tools bsdmainutils
\end{lstlisting}
...
\subsection{} На этом первоначальная настройка файловой системы окончена. Выйдите из chroot. 
...
Нажмите кнопку OK. Что бы в будущем не повторять настройку, можно сохранить текущею конфигурацию как начальную. Для этого выберите пункт Configuration->Save configuration.

Или подключитесь при помощи консольной программы minicom, для этого нажмите Ctrl+Shift+T для открытия новой вкладки в окне терминала и выполните команду 
