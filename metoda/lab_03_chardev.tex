...
\vspace{5mm}
\textbf{Описание:} В ОС Linux выделяется два пространства: пространство ядра (Kernel Space) и пользовательское пространство (User Space).
...
В ОС Linux выделяют три основных типа устройств:
\begin{itemize}
	...
	\item блочное устройство — представлено в файловой системе, минимальный объём это блок данных, работа ведётся путём монтирования устройства. Примеры: карты памяти, жёсткие диски и пр.  
...
# ssh-add
\end{lstlisting}
Впишите фразу заданную в пункте \ref{ssh:passphrase}
...
\begin{Notes}{Замечание:} 
	При первом входе, система может спросить о доверенности автора, нужно нажать на кнопку Yes, 
	\begin{center}
		\includegraphics[width=0.5\textwidth]{pic_13}
	\end{center}  
\end{Notes}

\subsection{} Добавьте путей для разрешения части include директив. Откройте .vscode -> c\_cpp\_properties.json 
...
\subsection{} Найдите поле includePath, поставьте запятую после ``\$\{workspaceFolder\}/**'' и допишите следующие строки:\\
\begin{center}
	\includegraphics[width=\textwidth]{pic_15}
\end{center}

для удобства, воспользуйтесь файлом vscode\_lines.txt в каталоге support. Ctrl+S для сохранения
...
\subsection{}Перейдите в терминал (можно открыть вкладку TERMINAL в vscode, если её не видно, выберите в меню Terminal -> New Terminal) и соберите модуль ядра командой
\begin{lstlisting}[style=bash]
...
Для работы с модулями в Linux есть ряд команд (доступны только пользователю root или через команду sudo):
...
\textbf{modporbe <module\_name>} - это «умная» версия insmod, проверяет зависимости (файл modules.dep), и подгружает их при необходимости. Требуется указывать только имя модуля, без расширения ko. При этом информация о модуле должна находиться в папке  /lib/modules/<kernel\_version>/
...
\subsection{} Подключитесь к плате через gtkterm или minicom (/dev/ttyUSB1 скорость 115200).
...
\subsection{} Создайте несколько файлов
\begin{lstlisting}[style=bash]
$ touch  /lib/modules/$(uname -r)/modules.order
$ touch  /lib/modules/$(uname -r)/modules.builtin
$ touch  /lib/modules/$(uname -r)/modules.builtin.modinfo
\end{lstlisting}
...
При этом пути к модулям, записанные в этих файлах ведутся от каталога, в котором они расположены.
...
\subsection{} Вернитесь в консоль виртуальной машины, и скопируйте скомпилированный модуль на плату (проверьте, что плата подключена сетевым шнуром к рабочему ПК)
\begin{lstlisting}[style=bash]
...
\subsection{} Определите зависимости для модуля,выполнив команду 
\begin{lstlisting}[style=bash]
$ depmod
\end{lstlisting}
...
\subsection{} Проверьте работу модуля, выполнив команду
\begin{lstlisting}[style=bash]
$ modprobe chardev
\end{lstlisting}
в результате должны увидеть надпись 
\begin{lstlisting}[style=stdout]
srisa_pdrv: srisa_pdrv_init
\end{lstlisting}

\subsection{} Посмотрите список загруженных модулей
\begin{lstlisting}[style=bash]
$ lsmod
\end{lstlisting}
...
Size - показывет сколько оперативной памяти выделено для выбранного модуля,

Used by - сколько программ или модулей используют на данный момент указаный модуль (очевидно показываются только активные приложения и модули). 

\subsection{} Выполните команду ls /dev/*. Обратите внимание, что отсутствует файл srisa\_pdrv, так как он создаётся, только при вызове функции pdrv\_probe. Так как в системе отсутствует привязка модуля к периферийному устройству, функция pdrv\_probe не вызывается.

\subsection{}Выгрузите из памяти модуль ядра
\begin{lstlisting}[style=bash]
$ rmmod chardev
\end{lstlisting}

\section{Наложенное дерево устройств}
Мы успешно собрали модуль устройства, однако его сейчас никто не использует. Как говорилось выше, часто модули ядра являются драйвером, для взаимодействия с периферией (как внутрипроцессорной, типа интерфейса spi, так и внешней). Для описания взаимодействия между ядром и устройством служит специальная структура - дерево устройств (device tree). В ней описывается основная информация о каждом устройстве, подключенным к процессорному ядру, а так же могут добавлять параметры, применяемые модулем ядра при работе с тем или иным устройством. Среди прочего, дерево устройств поддерживает функцию наложения (overlay), что позволяет создавать файлы, вносящие исправление в существующее описание, или дополняющее его, в соответствии с требованиями конкретного проекта, без необходимости пересобирать всё дерево устройств целиком.

\subsection{} Создайте рабочую папку в виртуальной машине для реализации наложенного дерева устройств и перейдите в неё
\begin{lstlisting}[style=bash]
# mkdir $BAGET/lab_03/devtree
# cd $BAGET/lab_03/devtree 
\end{lstlisting}

\subsection{} Создайте файл и откройте его для редактирования
\begin{lstlisting}[style=bash]
# kate of_mychard.dts
\end{lstlisting}
...
На данном шаге было создано дерево устройств, которое описывает новое виртуальное устройство, совместимое с драйвером, созданном ранее (обратите внимание на поле compatible в написанном фрагменте, и сравните его значение со значением одноименного параметра в исходном коде модуля). Ключевое слово overlay показывает, что данное дерево устройств должно быть наложено на уже существующее.  

\subsection{} Скомпилируйте наложенное дерево устройств, для этого выполните следующую команду 
\begin{lstlisting}[style=bash]
...
\vspace{5mm}
Особенность дерева устройств в том, что скомпилированное дерево устройств можно вернуть в «человекочитаемый код» командой 
\begin{lstlisting}[style=bash]
...
Модуль должен быть уже загружен, так как ядро при запуске должно было обнаружить  совместимое с модулем устройство. 

\subsection{} Посмотрите последние строки лога загрузки ядра
\begin{lstlisting}[style=bash]
$ dmesg | tail
\end{lstlisting}
...
Строку с init получали ранее, но теперь после неё есть строка srisa-pdrv: pdrv\_probe, которая показывает, что  модуль успешно связался с устройством описанным в дереве устройств.

\subsection{} Выполните команду ls /dev/* и обратите внимание, что появился файл srisa\_pdrv в выдаче.

\subsection{} Отправьте данные в srisa\_pdrv, и проанализируем сообщения в консоли
\begin{lstlisting}[style=bash]
...
\subsection{} Попробуйте считать данные из драйвера, для этого выполним следующую команду: 
\begin{lstlisting}[style=bash]
...
\subsection{} Оцените скорость чтения данных, но на этот раз перенаправим вывод в файл, в противном случае придётся долго ждать завершения вывода в консоль запрошенного объёма данных (или прервать его комбинацией клавиш Ctrl+C): 
\begin{lstlisting}[style=bash]
...
Командой dd можно передавать данные из любых файлов, что может помочь в проверке работы драйвера. Как один из вариантов, создайте текстовый файл, и впишите в него текст для передачи в драйвер. Через команду dd передаёте этот файл в драйвер, и второй командой dd считываете данные в другой файл, после чего можете сравнить два файла (исходный и полученный) командой diff <path1> <path2>
