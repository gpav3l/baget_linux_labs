...
\vspace{5mm}
\textbf{Описание:} В данной лабораторной работе, будет создан простой web сервер для мониторинга ресурсов, и управления железом удалённо. Данный проект покажет, как можно организовать графический пользовательский интерфейс для работы с железом, без необходимости тащить на плату гигабайты графических библиотек и поднимать драйверы для работы с дисплеями.  
...
В данной работе будет производиться работа с сервером Apache2 и CGI интерпретатором. В современной WEB разработке предпочтение отдают FastCGI или uWSGI в связке с nginx сервером, исходя из вопросов быстродействия. Однако настройка подобных систем более комплексная, и не относиться к вопросу проектирования встраиваемых систем. 

\subsection{} Перейдите в папку первой лабораторной работы, для добавления необходимых компонентов
...
\subsection{} Установите необходимые пакеты, веб-сервер apache2, расширение для работы cgi скриптов, а также pyhton3, так как CGI скрипты на bash создавать можно, но больно.
\begin{lstlisting}[style=bash]
# apt-get install -y \
apache2 \
python3 \
...
\subsection{} Активируйте модуль обработки cgi скриптов apache сервера
\begin{lstlisting}[style=bash]
# a2enmod cgi
\end{lstlisting}

\subsection{} По-умолчанию, cgi скрипты ищутся в папке /usr/lib/cgi-bin,  исправьте путь поиска на /var/www/cgi-bin, для чего откройте файл описания сайта:
\begin{lstlisting}[style=bash]
# nano /etc/apache2/sites-enabled/000-default.conf
\end{lstlisting}
и внутри ноды VirtualHost допишите
...
\subsection{} Завершите настройку rootfs
\begin{lstlisting}[style=bash]
# exit
\end{lstlisting}
...
\subsection{} Измените владельца для папки с содержимом сайта
\begin{lstlisting}[style=bash]
...
\subsection{} Необходимо узнать точку монтирования SD карты. Она может отличаться в зависимости от ридера. Для упрощения ввода, выполните команду в консоли:
\begin{lstlisting}[style=bash]
	# mount | grep ^/dev
\end{lstlisting}
Вывод будет примерно как на рисунке
...
Для упрощения ввода, создайте временную системную переменную, которая будет содержать путь к папке, для примера\textbf{(!)} на рисунке выше:
\begin{lstlisting}[style=bash]
	# export BAGET_SD=\
...
\begin{lstlisting}[style=bash]
	# sudo rm -rf $BAGET_SD/*
\end{lstlisting}
скопируйте обновленную файловую систему на карту памяти
\begin{lstlisting}[style=bash]
