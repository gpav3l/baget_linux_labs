...
Проверку работы SPI интерфейса будет проводится при помощи PMOD модуля компании Digilent PmodALS на которой установлен датчик освещённости, на базе микросхемы TEMT6000X01 подключенный к АЦП ADC081S021.
...
\begin{itemize}
	...
	\item \href{https://static.chipdip.ru/lib/311/DOC018311920.pdf}{Texas Instruments ADC081S021}
\end{itemize}
...
\subsection{} Если в окне терминала нет текста, нажмите клавишу Enter на клавиатуре. Должна появиться надпись следующего вида:
\begin{center}
...
\subsection{} Создайте рабочую папку в виртуальной машине для наложенного дерева устройств и перейдите в неё 
\begin{lstlisting}[style=bash]
# mkdir -p $BAGET/lab_05/devtree
# cd $BAGET/lab_05/devtree
\end{lstlisting}

\subsection{} Создайте новый файл
\begin{lstlisting}[style=bash]
# kate of_spi1.dts
\end{lstlisting}
и запишите в него следующий текст
...
Следующий фрагмент правок касается дополнения к описанию spi контроллера. Первое, это его активация, записью в параметр status значения okay. Так же, вводиться два дополнительных параметра. Первый хранит в себе название конфигурации выводов, второй — для указания идентификатора конфигурации. Обратите внимание, что значение у него совпадает (и это важно) со значением phandle у описания выводов из первого фрагмента.

Последняя строка гарантирует, что после применения этого дополнения, значение параметра pinctrl-0 будет соответствовать значению phandle группы spi1\_pins. Дело в том, что значение phandle из первого фрагмента, будет  актуализировано, для разрешения коллизий (так как в исходном device-tree уже есть нода, с таким значением phandle), и последняя строка в описании исправлений поясняет системе, что после разрешения коллизии, необходимо обновить значение указанного параметра (значение 0 зарезервировано, и в данном контексте означает ссылку на значение параметра phandle).  
...
\end{lstlisting}
Сохраните изменения и выйдите из редактора (Ctrl+S, Ctrl+X)
...
Из особенностей, которые хотелось бы отметить, это структура описание которой начинается с 11 строки. В ней описывается режим работы SPI интерфейса, в том числе номер интерфейса в системе. В данном случае 1, так как 0 занят контроллером QSPI памяти.
...
\subsection{} Перейдите в терминал платы, и активируйте драйвер
\begin{lstlisting}[style=bash]
...
В этом может помочь код из лабораторной работы \ref{lab:char_dev}.
