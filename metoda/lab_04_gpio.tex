...
\subsection{} Для начала подготовьте рабочую папку, для разработки наложенного дерева устройств для системы: 
\begin{lstlisting}[style=bash]
# mkdir -p $BAGET/lab_04/of_dtb/
# cd $BAGET/lab_04/of_dtb/
\end{lstlisting}

\subsection{} Создайте и откройте для правки файла 
\begin{lstlisting}[style=bash]
# kate ./of_gpio.dts
\end{lstlisting}
...
Вторая часть отвечает за настройку кнопки. В данном случае, при нажатии кнопки будет формироваться событие отправки символа Q (код кнопки 16). Полный список кодов кнопок можно посмотреть в файле по ссылке \ref{link:event_code} в начале лабораторной работы.

Параметр gpios определяет, какой вывод передаётся под управление драйвером. Входное значение состоит из трёх элементов:
\begin{enumerate}
...
\subsection{} Скомпилируйте наложенное дерево устройства, командой \label{lab_gpio:dts_compile}
\begin{lstlisting}[style=bash]
...
Работа с кнопкой ведется через детектирования событий генерирующихся входным устройством. В этом плане, схожим образом происходит взаимодействие с клавиатурой, мышкой или сенсорным экраном.  
...
\subsection{} Прервите работу программы, нажав Ctrl+C.

\subsection{} Дополнительно посмотрите, что на самом деле считывается из файла при работе с ним, для чего запустим конвейер из двух команд
\begin{lstlisting}[style=bash]
$ cat /dev/input/event0 | hexdump 
\end{lstlisting}
Тут уже при открытии Вы не увидите ничего, однако как только начнёте нажимать и отпускать кнопку, то увидите поток данных. В этом потоке храниться как информация о событии, так и временные метки, что видели ранее, инструмент evtest занимается преобразованием (парсингом) этого потока, в человекочитаемый вид. 

\subsection{} Прервите работу нажав Ctrl+C.

\section{Проверка работы светодиодов}

Светодиоды управляются через виртуальную файловую систему sysfs. 

\subsection{} Перейдите в следующий каталог на плате
\begin{lstlisting}[style=bash]
$ cd /sys/class/leds
\end{lstlisting}

\subsection{} Тут располагаются ссылки на все светодиоды доступные в системе. В нашем случае, должно быть две папки mmc0:: и led1, проверьте это выполнив команду  
\begin{lstlisting}[style=bash]
$ ls -l
\end{lstlisting}

\subsection{} Для включения светодиод отправьте 1 в файл brightness, и 0 для выключения. При подключении через ШИМ контроллер, этот файл позволяет устанавливать яркость светодиода (например, для управления яркостью подсветки экрана)
\begin{lstlisting}[style=bash]
...
Попробуем теперь использовать вывод общего назначения внутри собственного драйвера, что может часто понадобится, при работе с внешними аппаратными модулями. К примеру, для управления сигналом сброса или для переключения между режимами и т. д.

\subsection{} Скопируйте код демонстрирующий работу с GPIO в пространстве ядра. Вернитесь в консоль виртуальной машины, и выполните следующую команду
\begin{lstlisting}[style=bash]
# cp -r $BAGET/support/gpio_driver $BAGET/lab_04/driver
\end{lstlisting}

\subsection{} Изучите пример кода модуля ядра 
\begin{lstlisting}[style=bash]
...
\end{center}
\begin{Notes}{Замечание:} 
	Если этого файла нет, то необходимо горячими клавишами Shift+Ctrl+P вызвать поле ввода команд и выбрать C/C++: Edit Configuration (JSON) 
\end{Notes}
...
\end{center}
для удобства, воспользуйтесь файлом vscode\_lines.txt в папке support. Ctrl+S для сохранения.

\subsection{} Допишите compilerArgs, как на картинке ниже, для корректного перехода к реализации функций gpio драйвера ядра.\\
\begin{center}
	\includegraphics[width=0.5\textwidth]{pic_19}
\end{center}

\subsection{} Посмотрите содержимое файла gpio\_temp.c. В нём описан процесс подключения драйвера к выводу.

\subsection{} Откройте и посмотрите на Makefile. В нём есть раздел install который поможет Вам скопировать на целевую плату скомпилированный модуль ядра. При этом, перед копированием будет запущена сборка всего проекта. Это полезная практика, так как гарантирует Вам, что при исправлении мелкой опечатки, на плату попадёт код с исправлением, а не его предыдущая версия.

\subsection{} Перейдите в терминал (можно открыть вкладку TERMINAL в vscode, если её не видно, выберите в меню Terminal → New Terminal) и соберите модуль командой:
\begin{lstlisting}[style=bash]
...
\end{lstlisting}
\begin{Notes}{Замечание:}
	Попробуйте сами разобраться, как в Makefile описан процесс копирования файла на плату. Это может Вам помочь, при выполнении самостоятельных работ.
\end{Notes}

\subsection{} Внесите изменения в наложенное дерево устройств 
\begin{lstlisting}[style=bash]
...
\subsection{} Скомпилируйте файл dts, для этого выполните следующую команду
\begin{lstlisting}[style=bash]
...
\end{lstlisting}
Если не было допущено ошибок, то в течении 20 секунд плата будет попеременно включать светодиоды F0 и F1
