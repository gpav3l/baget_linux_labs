...
#elif defined(CONFIG_CPU_K64S)

/*
 * K64S hazards.
 * o Accesses to CP0 TLB regs and accesses to TLB (by tlbwr, tlbwi, tlbp,
 *   or by an address translation) must not occur one after other for 2 cpu
 *   cycles (must be guarded by 2 ssnops).
 * o Change of the CPU state (e.g. irq enabling/disabling) unaffects insns
 *   that have been passed to the execution stage
 */
